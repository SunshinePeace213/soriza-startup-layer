...
them (mirrors /customer-discovery's score_criteria.py: the bookkeeping is mechanical so it can't
be soft-pedaled by eye). The drift audit then measures the concept against the reconstructed
baseline (hypothesis + these deltas), never the stale file.
...
Usage:
  python3 build_delta_ledger.py <idea_dir>            # e.g. docs/ideas-stages/personalized-ai-digest
...
def main():
    ...
    ap.add_argument("--docs", nargs="*", default=None,
                    help="override the doc filenames to scan (default: the three upstream docs)")
    args = ap.parse_args()
    ...
    flagged_deltas, warnings, docs_scanned = [], [], []
    verdict = verdict_source = None
    rounds, tripped, cleared, unsettled, overrides = [], [], [], [], []
    ...
    for name in doc_names:
        ...
        if name == "customer-discovery.md":
            ...
            overrides = [ln.strip() for ln in lines if OVERRIDE_RE.search(ln)]
    ...
    if "customer-discovery.md" not in docs_scanned:
        warnings.append("customer-discovery.md absent — Discovery verdict/criteria not read. "
                        "/solution-design's entry guard should have stopped before here.")
