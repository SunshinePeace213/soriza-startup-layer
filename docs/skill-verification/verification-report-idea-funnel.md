...
## Re-verification (post-fix, 2026-06-04)

**Scope:** Re-audit of the 3 touched dimensions — **resurrect-seam**, **schema-contract**, and **design-fidelity** — confirming the 7 fixes from the original report plus a regression check across the same seams.

**Re-verdict: PASS WITH WARNINGS.** The 7 original findings are resolved at their core, but re-verification confirmed **2 of 2 raw findings** on the touched seams (1 warning, 1 minor). The W1–W3 resurrect/appeal fix itself is correct and complete (a resurrected candidate now re-enters at its kill gate via `resumeFrom = STAGE_INDEX[killed_at]`, prior gates are reused, absent ids are reconstructed from the prior ledger, and unresolved ids are logged). The two confirmed findings are *new/remaining drift* on the same re-run seam the fix touched — not regressions in the fixed logic, but contract-vs-code gaps adjacent to it.

### Confirmed findings (2)

**RV1 (warning) — CONTEXT.md and SKILL.md still promise a "next-ranked survivor promotion" / "sub-cap survivors stay queued" re-run behavior the engine does not implement**
File: `/home/ringo/soriza-startup-layer-idea-funnel/CONTEXT.md`
Detail: The W1–W3 resurrect/appeal fix is correct and complete: a candidate whose id is in `args.resurrect` re-enters AT its kill gate (`resumeFrom = STAGE_INDEX[killed_at]`; gate0 guard `resumeFrom > 0`, gate1 guard `resumeFrom > 1`, gate2 always runs), prior gates are reused so the run is cheap, ids absent from the current seeds are reconstructed from the prior ledger (engine lines 133–150), and unresolved resurrect ids are logged (lines 151–153). All three resurrect cases (gate-0/1/2 kill) re-enter at the correct gate. BUT the contract docs make a SECOND, separate claim that is still unimplemented. CONTEXT.md line 114 (Funnel ledger): "killed Candidates stay killed, sub-cap survivors stay queued, and a re-run promotes the next-ranked survivor without re-running settled desk gates." SKILL.md line 84: "settled Candidates aren't re-run (cheap re-runs), and a re-run promotes the next-ranked survivor when the current one resolves." This describes a NON-resurrect re-run that promotes a prior sub-cap survivor into a freed shortlist slot using its already-settled desk verdicts. The engine only wires `resumeFrom` / cheap re-use for resurrected (killed → appealed) candidates via `resurrectedFull`, which the ledger-reader populates ONLY for `args.resurrect` ids (engine line 81; ledger-reader.md line 24). Prior sub-cap survivors (status `advancing`, not killed) are returned only in the lightweight `settled` array (id/status/killed_at/score) with no gates and no `resumeFrom`. They are never re-injected into `candidates` (built purely from `seeds`, line 128) and the cap is recomputed only over `survivors` = candidates processed THIS run (lines 208–211). So a queued sub-cap survivor that is not re-supplied in seeds and not resurrected is dropped from ranking entirely (carried only as a `priorOnly` board row, line 233); if it IS re-supplied in seeds it re-runs every desk gate from Gate 0 (no `resumeFrom`, not cheap). Either way the documented "promotes the next-ranked survivor without re-running settled desk gates" behavior does not occur. This is remaining docs-vs-code drift on the same re-run seam the W1–W3 fix was meant to reconcile. The drift is also internally inconsistent: `.claude/skills/idea-funnel/references/ledger-schema.md` lines 67–74 ("Re-run / resurrect contract") describes the REAL behavior and contains NO promotion promise, directly contradicting CONTEXT.md line 114.
Fix: Pick one — (a) implement queued-survivor promotion: have ledger-reader also return FULL records (with gates) for prior `status=='advancing'`/`'shortlisted'` survivors, re-inject them into `candidates` with `resumeFrom` set so their settled desk gates are reused, and rank them in the fresh cap; or (b) if out of scope, narrow CONTEXT.md line 114 and SKILL.md line 84 to the behavior actually built — only an explicit `args.resurrect` re-enters cheaply at its kill gate; a plain re-run does not auto-promote sub-cap survivors.

**RV2 (minor) — Gate-2 resurrect depends on the persisted `gate1_sharpen` carrying the full hypothesis object, but ledger-schema.md documents only `hypothesis_path` and the gate-2 prompts never read `hypothesis.md` from disk**
File: `/home/ringo/soriza-startup-layer-idea-funnel/.claude/workflows/idea-funnel-engine.js`
Detail: When a candidate killed at Gate 2 is resurrected, the engine skips gate1 (`resumeFrom > 1`, line 170) and at gate2 reads the hypothesis ONLY from the reconstructed in-memory gates: `const hypo = c.gates.gate1_sharpen && c.gates.gate1_sharpen.hypothesis` (line 185), then passes `hypothesis=hypo` inline to the objection lenses (line 187), market-researcher (line 193), and judge (line 197). None of those prompts instructs reading `${ns}hypothesis.md` from disk (verified objection-lens.md and market-researcher.md have no disk fallback). The reconstructed `c.gates` comes from `rc.gates` returned verbatim by ledger-reader from the prior `ledger.json`. The sharpen-gate emits a full hypothesis object `{who, how_often, how_severe, status_quo, sentence}` (sharpen-gate.md line 42), and the engine hands the full verdict to the ledger-writer (line 235) which is told to "write exactly what you're given" — so the object is PROBABLY persisted. But the writer is ALSO told to "Follow ledger-schema.md exactly", and ledger-schema.md's `gate1_sharpen` example (line 45) shows only `{verdict, score, reason, hypothesis_path}` with no hypothesis object, so a writer (haiku) following the documented schema literally would omit it. In that case a Gate-2 resurrect runs the lenses, market sweep, and judge with `hypothesis=undefined`, silently degrading the re-judgment. The hypothesis.md file is on disk (`hypothesis_path`), so the safe fix is cheap. The defect is latent and resurrect-path-specific (matching minor severity); the contract is non-deterministic by design.
Fix: Either (a) add a disk fallback to the gate2 prompts on the resurrect path — instruct the lenses/market-researcher/judge to read `${ns}hypothesis.md` when the inline hypothesis is empty; or (b) make ledger-schema.md's `gate1_sharpen` example explicitly include the full hypothesis object (not just `hypothesis_path`) so the writer persists what the Gate-2 resurrect needs. This closes the latent gap where a gate-2 appeal re-judges against an empty hypothesis.
