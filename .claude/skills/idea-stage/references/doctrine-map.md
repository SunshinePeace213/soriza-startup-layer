...
> `expert-lens-map.md` (the disconfirm persona roster) and `stage-pipeline.md` (order + entry-guards).
...
| **disconfirm** | persona lenses (via `expert-lens-map.md`) **+** `forward-deployed-founder` **+** `lean-startup` | the always-on doctrine angles: the *no-distribution / SaaS-dead* objection (FDF) and the *riskiest-assumption* objection (lean) |
| **market-map** | `forward-deployed-founder` | the moat / distribution read alongside demand + competitors + sizing + trends |
| **solution-design** | `lean-startup` + `forward-deployed-founder` + `solo-founder` | MVP type & pivots; does the concept own a workflow *and* a channel; is the scope a one-person job |
| **idea-stage-exit** | `lean-startup` | the "**should we even build this?**" gate + the value/growth check against pre-registered kill-criteria |
...
- **Inline read, not invocation.** A stage opens the doctrine skill's content and applies it; it does not
  spawn an agent or call the `Skill` tool. This mirrors how `disconfirm` reads `expert-lens-map.md`.
- **Persona lenses are routed separately.** The `disconfirm` persona panel (fixed core-3 Thiel/Munger/
