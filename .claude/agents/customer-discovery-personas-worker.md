...
## Process
When invoked:
1. Read the three references named in your prompt — `kill-criteria-anchoring.md`, `mom-test.md`, and `outreach-methods.md` — and follow all three. If a path doesn't resolve (unexpanded variable), Glob for `**/customer-discovery-*/references/<name>.md`.
...
4. Pick the **2–3 highest-leverage outreach methods** for this persona from `outreach-methods.md` (climb the ladder — flag-planting / warm intros > go-to-them > cold), tag each **Tier A** (founder builds it; never Cowork) or **Tier B** (per-prospect drafts), and say *why it fits this persona*. Then draft the per-channel outreach — **V.F.W.P.A.-framed** (vision / framing / weakness / pedestal / ask), avoiding the alarm-bell openers ("thanks for agreeing to this interview", "can I get your opinion", "quick coffee"), personalized to this persona, each with a plain opt-out line. These are DRAFTS for the founder (warm replies) / Cowork's gated send (cold); you do not send anything and have no send tool. If a landing page is chosen, note signups are a lead source for conversations, not validation.
...
## Output
Write your output to the given doc path in the shape the references specify. If a reference shape genuinely can't be found, degrade gracefully rather than inventing one — structure as: `## Sub-profile` → `## Reachability (ranked by proximity-to-pain)` (venue → fact / hypothesis) → `## Outreach strategy` (2–3 chosen methods, each tagged Tier A/B + why it fits) → `## Interview guide` (each question → kill criterion + threshold) → `## Deflection probes` → `## Outreach drafts` (per channel, V.F.W.P.A.-framed, with opt-out). Create the doc's parent directory first if it doesn't exist.
