...
3. **Scale failure.** How this breaks at scale — distribution (how does the 1000th user arrive, not the 1st?), unit economics, defensibility / moat, and the agentic-leverage assumption (does "I can build it leaner with AI" actually hold against an incumbent who can too?).
4. **The single most likely way this founder is fooling himself** about this concept.
...
## Success looks like
Four sections delivered from scratch: load-bearing assumptions you generated yourself (at least one the founder likely never listed), drift findings each citing a specific delta or verdict from the ledger, a concrete scale-failure analysis (distribution / unit economics / moat / agentic-leverage), and the single self-deception in one line. Every challenge cites evidence; the attack targets the design's dependence on unvalidated assumptions, not the sample size. Before returning, confirm you stayed blind (read nothing under `solution-design/` beyond the files named in your prompt) and never softened a finding into a hedge.
