...
trip-or-clear the kill criteria pre-registered at G4 from `/pressure-test` α's `pressure-report-alpha.md`**
— not to "learn about users" in the abstract. Each **OPEN assumption** + its matching **interview
question** in that α report became one criterion, and every synthesis scores real behaviour against a
*locked* threshold.

## The anchoring pattern

For each OPEN assumption (with its interview question) in `pressure-report-alpha.md`, build:

```
OPEN ASSUMPTION (verbatim from pressure-report-alpha.md open_assumptions[].assumption)
  → INTERVIEW QUESTION        (the α report's matching interview_questions[] entry — already past-behaviour shaped)
...
The interview questions in the α report are already Mom-Test / past-behaviour shaped (the pressure-test α
...
Worked examples (for an `personalized-ai-digest`-style idea — use the founder's real α report):

> **Open assumption:** *"What would have to be true: users will pay a non-zero price against free
> alternatives."*
> **Interview question (from the α report):** *"Tell me about the last time you paid for something to keep up
...
> **Interview question (from the α report):** *"Walk me through what you actually did in the last 7 days to keep up with AI — did you try a tool or ship something, or only read?"*
...
These criteria are encoded once **at G4** by `/pressure-test` α (write-once), from the OPEN assumptions;
synthesis (5b) **consumes** them and never re-locks. After interview data exists, **never edit a
threshold** — that is the goalpost-moving this whole design prevents. This is the schema the α step writes
to: each OPEN assumption in `pressure-report-alpha.md` is phrased as "What would have to be true: …" — the
scoring `type` is picked from what the assumption claims and the threshold set at G4 when locking it:
...
```json
{
  ...
  "criteria": [
    ...
    {"id": "fomo-not-need", "label": "Behaviour is FOMO, not need",
     "source": "pressure-report-alpha.md open assumption #2",
    ...
    {"id": "native-good-enough", "label": "Native personalization is good enough",
     "source": "pressure-report-alpha.md open assumption #3",
     "type": "against_majority", "field": "native_suffices", "min_n": 8},
    {"id": "no-capturable-signal", "label": "No capturable signal the labs lack",
     "source": "pressure-report-alpha.md open assumption #4", "type": "manual"}
  ]
}
```

## `scoring.json` — the per-interview tags (Step S3)
...
(Step S4 — pass full `ideas/<slug>/…` paths; the script runs from the repo root). It applies each locked threshold and returns per-criterion `TRIPPED` / `CLEARED` /
