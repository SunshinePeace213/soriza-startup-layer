...
- **Input:** `ideas/<slug>/customer-discovery.md` (required floor) + `hypothesis.md`, `disconfirmation-brief.md`, `market-research.md` (folded if present) + `docs/founder-profile.md`.
- **Output:** `ideas/<slug>/solution-design.md` (the action card) + provenance under `solution-design/`.
...
- Founder has a `customer-discovery.md` at `ideas/<slug>/` and asks "what's next"
...
Out of scope: editing `hypothesis.md` (`/sharpen-hypothesis` owns it); market sizing / competitor mapping
(`/market-map`); the adversarial expert disconfirmation (folded into the `disconfirmation-brief.md`
`/disconfirm` writes); running or synthesizing interviews (`/customer-discovery-synthesis`); the GO/NO-GO
(`/idea-stage-exit`) and PoC/build work (`/build-poc`, the next stages).
...
- **The Concept Read is non-binding — do not overclaim it as a gate.** Like `/market-map`'s read, it's a recommendation, not a pass/fail verdict the founder must "accept." Proceeding past a KILL is the only thing stamped as an override.
...
- **No persona panel here.** The `*-perspective` advisors live exclusively in `/disconfirm`'s expert panel (the `disconfirmation-brief.md`). You may run ONE named perspective skill as an a-la-carte design lens if the founder asks (e.g. `/steve-jobs-perspective` for "is this focused / whole-widget?") — but never reconstruct the panel or its verdict machinery.
- **Don't re-run market research or discovery.** WebSearch/WebFetch are for *light* verification only (does an "alternative" already exist; is a scale-economics claim sane). Re-deriving the competitive landscape (`/market-map`) or re-interviewing is scope creep into the prior stages.
...
style; read `/customer-discovery-synthesis` for voice). One phase, two gates — there is no
...
If invoked with a slug, use it. Otherwise `glob ideas/*/` for ideas with a
`customer-discovery.md`: exactly one → use it (state which in chat); more than one → `AskUserQuestion`;
none → if idea folders exist but none has `customer-discovery.md`, point at `/customer-discovery-synthesis`; if no
idea folders, point at `/generate-ideas`. Stop.
...
`ideas/<slug>/customer-discovery.md`. Run /customer-discovery-synthesis first — solution design must
...
If `ideas/<slug>/solution-design.md` already exists, fire `AskUserQuestion`:
- **Overwrite from scratch** — `rm -rf ideas/<slug>/solution-design/` and proceed.
...
Create the output directory first: `ideas/<slug>/solution-design/`.

Run the ledger script so no flagged delta is dropped before reasoning:
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/build_delta_ledger.py" ideas/<slug>
```
...
`assets/solution-design-template.md` for the shape.
...
Read `assets/solution-design-template.md`. Compose `solution-design.md` (draft in chat first) with all
...
- `build` / `narrow`: *"Solution design written. Next: run /idea-stage-exit (the GO/NO-GO), then /build-poc consumes the MVP Brief — the wedge, the prioritized riskiest-assumption-tests, the validated profile, the non-goals. Build the cheapest test of the #1 assumption first."*
...
- `reconsider`: *"The load-bearing assumptions aren't supported (or discovery was KILL). Step back: more /customer-discovery-synthesis on the riskiest assumptions, or re-run /sharpen-hypothesis to rework the problem."*
- If **Hypothesis Updates Flagged** is non-empty: *"The design flagged hypothesis updates — re-run /sharpen-hypothesis on <slug> to apply them, then re-run the downstream stages."*
...
```
You are the solution-red-team for a committed startup idea that has been through the idea stage (sharpen-hypothesis, expert disconfirmation, market-map) and customer-discovery-synthesis. You are NOT a balanced reviewer — your job is to find where this solution concept fails and where the founder is fooling themselves. You are working BLIND: you do not get the founder's drafted design card, only the validated evidence and the bare concept wedge, so that your assumptions and drift findings are independent.
...
Write your challenge to: ideas/<SLUG>/solution-design/red-team.md
Return one line naming the concept + the doc path — not the contents.
```

## Reference files

- `assets/solution-design-template.md` — the 9-section `solution-design.md` card structure. Read in Steps 4 and 6.
...
- `scripts/rank_assumptions.py` — reads `assumptions.json` (per-assumption leverage + uncertainty, 1–5), computes leverage×uncertainty, ranks, features the top 3, and flags adversary-sourced assumptions that reach the top 3. Bundled so the ranking can't be fudged by eye (mirrors `/customer-discovery-synthesis`'s `score_criteria.py`). Stdlib-only.

## Composition

- **Upstream:** `/customer-discovery-synthesis` writes the required `customer-discovery.md`; `/sharpen-hypothesis` (`hypothesis.md`), `/disconfirm` (`disconfirmation-brief.md`), and `/market-map` (`market-research.md`) supply the assumptions, positioning, and the assumed baseline. Floor guard = `customer-discovery.md`.
- **Downstream:** `/idea-stage-exit` gates the build (GO/NO-GO), then `/build-poc` consumes `solution-design.md`'s **MVP Brief** — the wedge, the prioritized riskiest-assumption-tests, the validated profile, the non-goals. Hypothesis updates route back through `/sharpen-hypothesis`; a `reconsider` Read routes back to `/customer-discovery-synthesis`.
...
- **Out:** editing `hypothesis.md` (`/sharpen-hypothesis` owns it). Market sizing / competitor mapping (`/market-map`). The persona panel (`/disconfirm`'s expert disconfirmation → `disconfirmation-brief.md`). Running/synthesizing interviews (`/customer-discovery-synthesis`). The GO/NO-GO (`/idea-stage-exit`) and PoC/build work (`/build-poc`). Inventing persona skills (`/nuwa-skill`).
